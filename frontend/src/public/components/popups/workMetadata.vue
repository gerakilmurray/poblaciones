  ...
  methods: {
		...
		resolveFileUrl(file) {
			if (file.Web) {
				...
				return '/services/metadata/GetMetadataFile?m=' + this.work.MetadataId + '&f=' + file.FileId;
			} else {
				return '#';
			}
		},
		resolveMetadataUrl() {
			return '/services/metadata/GetMetadataPdf?m=' + this.work.MetadataId + (this.level ? '&d=' + this.level.Dataset.Id : '') + '&w=' + this.work.Id;
		},
	},
	computed:
	{
